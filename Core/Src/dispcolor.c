...
{
	if (x1 == x2) {
		if (y1 > y2) dispcolor_FillRect(x1, y2, 1, y1 - y2 + 1, color);
		        else dispcolor_FillRect(x1, y1, 1, y2 - y1 + 1, color);
		return;
	}

	if (y1 == y2) {
		if (x1 > x2) dispcolor_FillRect(x2, y1, x1 - x2 + 1, 1, color);
		        else dispcolor_FillRect(x1, y1, x2 - x1 + 1, 1, color);
		return;
	}
...
const int16_t signY = y1 < y2 ? 1 : -1;
	...
	if (thick > 1) dispcolor_FillCircle(x2, y2, thick >> 1, color);
	          else dispcolor_DrawPixel(x2, y2, color);

	while (x1 != x2 || y1 != y2) {
		if (thick > 1) dispcolor_FillCircle(x1, y1, thick >> 1, color);
		          else dispcolor_DrawPixel(x1, y1, color);
...
//------------------------------------------------------------------------------
static void DrawLine_Slow_Wu(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
...
float last_fade_amount = 0;
	...
	while (i < j) {
		...
		if (fade_amount < last_fade_amount) j--;	// Opaqueness reset so drop down a row
...
{
	if (radius > 1) dispcolor_FillCircle(x0, y0, radius - 1, color);
...
int16_t xLast = -1, yLast = -1;
	...
	for (int16_t angle = startAngle; angle <= endAngle; angle += 2) {
		...
		if (thick > 1) dispcolor_DrawLine2(xLast, yLast, x, y, color, thick);
		          else dispcolor_DrawLine(xLast, yLast, x, y, color);
...
char StrBuff[100];
	...
	return dispcolor_DrawString_Bg(X, Y, FontID, StrBuff, TextColor, BgColor);
}
//------------------------------------------------------------------------------
